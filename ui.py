...
class UI:
    ...
    def show_exp(self, exp):
        text_surf = self.font.render(f"{exp}", True, TEXT_COLOR)
    ...
    def weapon_overlay(self, wepon_index, has_switched):
        bg_rect = self.selection_box(10, 550, has_switched) # weapon
        wepon_surf = self.weapon_graphics[wepon_index]
        wepon_rect = wepon_surf.get_rect(center = bg_rect.center)
        self.display_surface.blit(wepon_surf, wepon_rect)

    def magic_overlay(self, magic_index, has_switched):
        bg_rect = self.selection_box(80, 560, has_switched) # magic
